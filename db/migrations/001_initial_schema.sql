...
-- Description: Initial schema for Polybot trade tracking
-- Version: 1.0.0
--
-- This migration creates:
-- - Enum types for trade status, trade side, and order side
-- - Tables for wallets, markets, and trades
...
CREATE INDEX IF NOT EXISTS idx_markets_resolved ON markets(resolved) WHERE resolved = false;

-- Trade queries
CREATE INDEX IF NOT EXISTS idx_trades_wallet_id ON trades(wallet_id);
CREATE INDEX IF NOT EXISTS idx_trades_market_id ON trades(market_id);
CREATE INDEX IF NOT EXISTS idx_trades_status ON trades(status);
CREATE INDEX IF NOT EXISTS idx_trades_wallet_status ON trades(wallet_id, status);
CREATE INDEX IF NOT EXISTS idx_trades_created_at ON trades(created_at);

-- Composite index for common queries
CREATE INDEX IF NOT EXISTS idx_trades_wallet_market ON trades(wallet_id, market_id);
