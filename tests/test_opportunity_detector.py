...
class TestOpportunityDataclass:
    ...
    def test_opportunity_equality(self):
        ...
        now = datetime(2024, 1, 15, 10, 13, 0)
        opp1 = Opportunity("test", "YES", 0.75, now, "last_trade")
    ...
    def test_opportunity_inequality_price(self):
        ...
        now = datetime(2024, 1, 15, 10, 13, 0)
        opp1 = Opportunity("test", "YES", 0.75, now, "last_trade")
        opp2 = Opportunity("test", "YES", 0.80, now, "last_trade")
        assert opp1 != opp2

    def test_opportunity_inequality_market_id(self):
        """Verify Opportunity instances with different market IDs are not equal."""
        now = datetime(2024, 1, 15, 10, 13, 0)
        opp1 = Opportunity("test1", "YES", 0.75, now, "last_trade")
...
class TestDetectOpportunity:
    ...
    def test_no_opportunity_below_threshold(self):
        """Verify no opportunities when price below threshold."""
        opportunities = detect_opportunity(
    ...
    def test_no_opportunity_just_below_threshold(self):
        ...
        opportunities = detect_opportunity(
            last_trade_price=0.699999,
            threshold=0.70,
            market_id="btc-15min",
        )
        assert len(opportunities) == 0

    def test_no_opportunity_with_none_last_trade_price(self):
        """Verify no opportunity when last trade price is None."""
        opportunities = detect_opportunity(
    ...
    def test_opportunity_with_nan_last_trade_price(self):
        """Verify no opportunity when last trade price is NaN."""
        opportunities = detect_opportunity(
    ...
    def test_opportunity_with_negative_last_trade_price(self):
        """Verify no opportunity when last trade price is negative."""
        opportunities = detect_opportunity(
    ...
    def test_opportunity_side_is_yes(self):
        """Verify opportunity has side set to YES."""
        opportunities = detect_opportunity(
...
class TestDetectOpportunitiesBatch:
    ...
    def test_batch_single_market_no_opportunity(self):
        ...
        price_data = [
            {"market_id": "btc-15min", "last_trade_price": 0.60},
        ]
        opportunities = detect_opportunities_batch(price_data, threshold=0.70)
        assert len(opportunities) == 0
    ...
    def test_batch_multiple_markets_mixed(self):
        ...
        price_data = [
            ...
            {"market_id": "sol-15min", "last_trade_price": 0.68},
        ]
        opportunities = detect_opportunities_batch(price_data, threshold=0.70)

        # btc: 1 opportunity (above threshold), eth: 0, sol: 0
        assert len(opportunities) == 1

        market_ids = {opp.market_id for opp in opportunities}
        assert "btc-15min" in market_ids
        assert "sol-15min" not in market_ids
    ...
    def test_batch_all_markets_with_opportunities(self):
        ...
        price_data = [
            {"market_id": "btc-15min", "last_trade_price": 0.75},
            {"market_id": "eth-15min", "last_trade_price": 0.82},
            {"market_id": "sol-15min", "last_trade_price": 0.88},
        ]
        opportunities = detect_opportunities_batch(price_data, threshold=0.70)
    ...
    def test_batch_no_markets_with_opportunities(self):
        ...
        price_data = [
            {"market_id": "btc-15min", "last_trade_price": 0.50},
            {"market_id": "eth-15min", "last_trade_price": 0.58},
            {"market_id": "sol-15min", "last_trade_price": 0.62},
        ]
        opportunities = detect_opportunities_batch(price_data, threshold=0.70)
        assert len(opportunities) == 0
    ...
    def test_batch_handles_none_values(self):
        ...
        price_data = [
            {"market_id": "btc-15min", "last_trade_price": None},
        ]
        opportunities = detect_opportunities_batch(price_data, threshold=0.70)
        assert len(opportunities) == 0
    ...
    def test_batch_order_preserved(self):
        ...
        price_data = [
            {"market_id": "aaa-market", "last_trade_price": 0.75},
            {"market_id": "bbb-market", "last_trade_price": 0.80},
            {"market_id": "ccc-market", "last_trade_price": 0.85},
        ]
        opportunities = detect_opportunities_batch(price_data, threshold=0.70)
...
class TestDetectOpportunityEdgeCases:
    ...
    def test_infinity_last_trade_price(self):
        """Verify infinity last trade price is handled as invalid."""
        opportunities = detect_opportunity(
            last_trade_price=float("inf"),
            threshold=0.70,
            market_id="test",
        )
        # Infinity is invalid, no opportunity detected
        assert len(opportunities) == 0
    ...
    def test_negative_threshold(self):
        ...
        # Positive price should be >= -0.50
        assert len(opportunities) == 1
        assert opportunities[0].source == "last_trade"

    def test_price_exactly_zero(self):
        """Verify price at exactly zero with zero threshold."""
        opportunities = detect_opportunity(
...
class TestOpportunityTimestamp:
    ...
    def test_opportunity_timestamp_type(self):
        """Verify opportunity timestamp is a datetime instance."""
        opportunities = detect_opportunity(
            last_trade_price=0.80,
            threshold=0.70,
            market_id="test",
